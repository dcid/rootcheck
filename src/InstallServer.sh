...
USER_MAIL="ossecm"
USER_REM="ossecr"
subdirs="logs logs/archives logs/alerts logs/firewall bin stats rules queue queue/alerts queue/ossec queue/fts queue/syscheck queue/rootcheck queue/diff queue/agent-info queue/agentless queue/rids tmp var var/run etc etc/shared active-response integrations active-response/bin agentless .ssh"
...
chmod -R 775 ${DIR}/queue/agent-info
chmod 774 ${DIR}/queue/agent-info/* > /dev/null 2>&1
...
cp -p ../integrations/* ${DIR}/integrations/
chmod 755 ${DIR}/integrations/*
chown root:${GROUP} ${DIR}/integrations/*

touch ${DIR}/logs/integrations.log
chmod 775 ${DIR}/logs/integrations.log
